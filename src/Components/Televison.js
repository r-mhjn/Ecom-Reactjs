...
export default class Television extends React.Component {
  ...
  _getProducts = async () => {
    await Axios.get(`http://${ip.default}:5050/user/products/televisions`)
      .then(res => {
        this._saveProducts(res.data[0]);
        console.log(this.state.products);
      })
      .catch(err => console.log("Error while fetching televisions " + err));
  };
            ...
            <div className="row">
              {this.state.products.map((item, index) => {
                return (
                  <div
                    key={index}
                    className="col-xs-6 col-sm-4 col-lg-4 col-md-4"
                  >
                    <div
                      className="card"
